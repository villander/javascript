...
new XMLHttpRequest;b.onerror=b.onabort=function(){h(1,b.responseText||{error:"Network Connection Error"})};b.onload=b.onloadend=e;b.onreadystatechange=function(){if(b&&4==b.readyState)switch(b.status){case 401:case 402:case 403:case 400:case 500:case 502:try{c=JSON.parse(b.responseText),h(1,c)}catch(a){return h(1,b.responseText)}}};var m=sa(a.url,u);b.open("GET",m,f);f&&(b.timeout=j);b.send()}catch(g){h(0);Cb=0;a=Fb(a);break a}a=h}return a}var l=Bb("script"),e=a.a,D=ma(),x=0,Ja=setTimeout(function(){d(1,
{message:"timeout"})},a.timeout||1E4),Y=a.b||A(),j=a.data||{},ab=a.c||A();window[e]=function(a){d(0,a)};a.h||(l[Gb]=Gb);l.onerror=function(){d(1)};l.src=sa(a.url,j);zb(l,"id",D);xb().appendChild(l);return d},Hb=function(){if(!("onLine"in navigator))return 1;try{return navigator.onLine}catch(a){return v}},Z=function(){if(!Ib||!Ib.get)return 0;var a={id:Z.id++,send:A(),abort:function(){a.id={}},open:function(d,b){Z[a.id]=a;Ib.get(a.id,b)}};return a},Gb="async",Ob=navigator.userAgent,Cb=-1==Ob.indexOf("MSIE 6");
window.console||(window.console=window.console||{});console.log||(console.log=console.error=(window.opera||{}).postError||A());var Pb,Qb={},Rb=z;try{Rb=window.localStorage}catch(Sb){}var Tb=function(a){return-1==document.cookie.indexOf(a)?y:((document.cookie||"").match(RegExp(a+"=([^;]+)"))||[])[1]||y},Ub=function(a,d){document.cookie=a+"="+d+"; expires=Thu, 1 Aug 2030 20:00:00 UTC; path=/"},Vb;try{Ub("pnctest","1"),Vb="1"===Tb("pnctest")}catch(Xb){Vb=z}Pb={get:function(a){try{return Rb?Rb.getItem(a):
Vb?Tb(a):Qb[a]}catch(d){return Qb[a]}},set:function(a,d){try{if(Rb)return Rb.setItem(a,d)&&0;Vb&&Ub(a,d);Qb[a]=d}catch(b){Qb[a]=d}}};var Yb={list:{},unbind:function(a){Yb.list[a]=[]},bind:function(a,d){(Yb.list[a]=Yb.list[a]||[]).push(d)},fire:function(a,d){M(Yb.list[a]||[],function(a){a(d)})}},Zb=tb("pubnub")||0,ac=function(a){function d(){}function b(a,b){function c(b){b&&(db=G()-(b/1E4+(G()-d)/2),a&&a(db))}var d=G();b&&c(b)||B.time(c)}function c(a,b){Oa&&Oa(a,b);Oa=y;clearTimeout($);clearTimeout(R)}
function e(){bc&&B.time(function(a){b(A(),a);a||c(1,{error:"Heartbeat failed to connect to Pubnub Servers.Please check your network settings."});R&&clearTimeout(R);R=setTimeout(e,Db)})}function p(){cc()||c(1,{error:"Offline. Please check your network settings. "});$&&clearTimeout($);$=setTimeout(p,E)}function q(a,b,c,d){var b=a.callback||b,e=a.error||t,f=I(),d=d||{};d.auth||(d.auth=a.auth_key||J);a=[L,"v1","channel-registration","sub-key",w];a.push.apply(a,c);f&&(d.callback=f);H({a:f,data:D(d),c:function(a){s(a,
b,e)},b:function(a){j(a,e)},url:a})}function j(a,b){if("object"==typeof a&&a.error){var c={};a.message&&(c.message=a.message);a.payload&&(c.payload=a.payload);b&&b(c)}else b&&b(a)}function s(a,b,c){if("object"==typeof a){if(a.error){b={};a.message&&(b.message=a.message);a.payload&&(b.payload=a.payload);c&&c(b);return}if(a.payload){a.next_page?b&&b(a.payload,a.next_page):b&&b(a.payload);return}}b&&b(a)}function C(a){var b=0;M(ya(F),function(c){if(c=F[c])b++,(a||A())(c)});return b}function u(a){var b=
0;M(za(N),function(c){if(c=N[c])b++,(a||A())(c)})}function r(a){if(dc){if(!aa.length)return}else{a&&(aa.n=0);if(aa.n||!aa.length)return;aa.n=1}H(aa.shift())}function f(){!eb&&h()}function h(){clearTimeout(Pa);!O||500<=O||1>O||!ya(F,v).length&&!za(N,v).length?eb=z:(eb=v,B.presence_heartbeat({callback:function(){Pa=setTimeout(h,O*E)},error:function(a){t&&t("Presence Heartbeat unable to reach Pubnub servers."+JSON.stringify(a));Pa=setTimeout(h,O*E)}}))}function m(a,b){return Qa.decrypt(a,b||V)||Qa.decrypt(a,
V)||a}function g(a,b,c){var d=z;if("number"===typeof a)d=5<a||0==a?z:v;else{if("boolean"===typeof a)return a?30:0;d=v}return d?(c&&c("Presence Heartbeat value invalid. Valid range ( x > 5 or x = 0). Current Value : "+(b||5)),b||5):a}function l(a){var b="",c=[];M(a,function(a){c.push(a)});var d=c.sort(),e;for(e in d){var f=d[e],b=b+(f+"="+encodeURIComponent(a[f]));e!=d.length-1&&(b+="&")}return b}function D(a){a||(a={});M(P,function(b,c){b in a||(a[b]=c)});return a}function x(a){return ac(a)}function Ja(a){function b(a,
...
"leave"]});return v},set_resumed:function(a){hb=a},get_cipher_key:function(){return V},set_cipher_key:function(a){V=a},raw_encrypt:function(a,b){return Qa.encrypt(a,b||V)||a},raw_decrypt:function(a,b){return m(a,b)},get_heartbeat:function(){return W},set_heartbeat:function(a){W=g(a,O,t);O=1<=W-3?W-3:1;d();h()},get_heartbeat_interval:function(){return O},set_heartbeat_interval:function(a){O=a;h()},get_version:function(){return"3.7.6"},getGcmMessageObject:function(a){return{data:a}},getApnsMessageObject:function(a){var b=
